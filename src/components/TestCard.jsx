export default function TestCard({ test, onToggle, isSelected }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition">
      <div className="flex justify-between items-start mb-2">
        <span className={`text-xs px-2 py-1 rounded-full ${test.fasting ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}>
          {test.fasting ? `Fasting Required • ${test.hours}` : 'No Fasting Required'}
        </span>
      </div>
      <h3 className="font-bold text-dark text-lg">{test.name}</h3>
