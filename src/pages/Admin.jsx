...
const ADMIN_EMAIL = 'kugoramoweyipehcaesar49@gmail.com'
const ADMIN_PASS = 'Dominion4244'
...
export default function Admin({ showToast }) {
  ...
  useEffect(() => {
    ...
    const saved = JSON.parse(localStorage.getItem('testPrices') || '{}')
  ...
  const handleAdminLogin = (e) => {
    ...
    if (email === ADMIN_EMAIL && password === ADMIN_PASS) {
      ...
      localStorage.setItem('currentUser', JSON.stringify({ email, fullName: 'Dr. Ama Mensah', role: 'admin' }))
      setIsLoggedIn(true)
  ...
  const updatePrices = () => {
    const current = JSON.parse(localStorage.getItem('testPrices') || '{}')
    const updated = { ...current, ...prices }
    localStorage.setItem('testPrices', JSON.stringify(updated))
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
          <h1 className="text-2xl font-bold text-dark text-center mb-6">Admin Login</h1>
          <form onSubmit={handleAdminLogin} className="space-y-4">
            <div>
              <label className="text-sm font-medium">Email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full border rounded-lg px-3 py-2.5 mt-1" required />
            </div>
            <div>
              <label className="text-sm font-medium">Password</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full border rounded-lg px-3 py-2.5 mt-1" required />
            </div>
            <button type="submit" className="w-full bg-primary text-white py-3 rounded-lg font-medium">Login as Admin</button>
          </form>
        </div>
      </div>
    )
        ...
        <nav className="flex-1 p-3 space-y-1">
          {sidebarItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${activeTab === item.id ? 'bg-primary' : 'hover:bg-gray-800'}`}
            >
              <item.icon size={18} />
              {item.label}
            </button>
      ...
      <main className="flex-1 p-6 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input placeholder="Search bookings, clients, tests..." className="pl-9 pr-4 py-2 border rounded-lg text-sm w-64" />
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl p-4 border">
            ...
            <p className="text-3xl font-bold text-dark">3,240</p>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl border p-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-bold text-dark">New Bookings</h2>
                    ...
                    <tr key={b.id} className="border-b last:border-0">
                      ...
                      <td className="py-3">
                        <div className="flex flex-wrap gap-1">
                          {(Array.isArray(b.tests) ? b.tests : [b.tests]).map(t => (
                            <span key={t} className="bg-blue-50 text-primary text-xs px-2 py-0.5 rounded">{t}</span>
                          ))}
                        </div>
                      </td>
                      ...
                      <td className="py-3">
                        <div className="flex gap-1">
                          <button onClick={() => showToast('Edit mode')} className="p-1.5 hover:bg-gray-100 rounded"><Edit2 size={14} /></button>
                          <button onClick={() => updateStatus(b.id, 'Confirmed')} className="p-1.5 hover:bg-gray-100 rounded text-blue-600"><UserPlus size={14} /></button>
                          <button onClick={() => updateStatus(b.id, 'Approved')} className="p-1.5 hover:bg-gray-100 rounded text-green-600"><Check size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border p-4">
              <h3 className="font-bold text-dark mb-3 flex items-center gap-2"><Edit2 size={16} /> Edit Test Prices</h3>
                ...
                <div key={name} className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      value={price}
                      onChange={e => setPrices(prev => ({ ...prev, [name]: Number(e.target.value) }))}
            ...
            <div className="bg-white rounded-xl border p-4">
              <h3 className="font-bold text-dark mb-3 flex items-center gap-2"><Upload size={16} /> Upload Results PDF</h3>
