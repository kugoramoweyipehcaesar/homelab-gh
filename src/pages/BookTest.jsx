...
export default function BookTest({ showToast, updateCartCount }) {
  ...
  useEffect(() => {
    ...
    setSelected(cart)
    const p = JSON.parse(localStorage.getItem('testPrices') || '{}')
    setPrices(p)
  }, [])
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-dark mb-6">Book a Lab Test</h1>

      <div className="flex mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex-1 text-center">
            <div className={`w-8 h-8 rounded-full mx-auto flex items-center justify-center text-sm font-bold ${i <= step ? 'bg-primary text-white' : 'bg-gray-200'}`}>
              {i + 1}
            </div>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={address} onChange={e => setAddress(e.target.value)} placeholder="House number, street, landmark" className="w-full border rounded-lg px-3 py-2" />
          </div>
          <div>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div>
            ...
            <input type="tel" value={momo} onChange={e => setMomo(e.target.value)} placeholder="+233 24 000 0000" className="w-full border rounded-lg px-3 py-2" />
          </div>
      ...
      <div className="flex justify-between mt-8">
        {step > 0 ? (
          <button onClick={() => setStep(step - 1)} className="px-5 py-2 border rounded-lg">Back</button>
        ) : <div />}
        {step < 3 ? (
          <button
