    ...
    <div className="max-w-lg mx-auto bg-white min-h-screen pb-20">
      ...
      <div className="p-4">
        <div className="bg-blue-50 rounded-xl p-4 mb-6">
          <h1 className="text-2xl font-bold text-dark">Hello, {firstName}!</h1>
        ...
        <h2 className="font-bold text-dark mb-3">Test Progress</h2>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white border rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-dark">{bookings.length || 12}</p>
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => navigate('/my-results')} className="text-sm text-primary">See all</button>
        </div>
        <div className="space-y-2 mb-6">
          {(bookings.length ? bookings.slice(0, 3) : [
            ...
            <div key={i} className="flex items-center justify-between bg-white border rounded-xl p-3">
              <div>
                <p className="font-medium text-sm">{Array.isArray(b.tests) ? b.tests.join(', ') : b.tests}</p>
                <p className="text-xs text-gray-500">{b.date || b.createdAt}</p>
              </div>
              <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">{b.status || 'Completed'}</span>
            </div>
          ))}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around py-2 max-w-lg mx-auto">
        ...
        <button onClick={() => navigate('/tests')} className="flex flex-col items-center text-gray-500 text-xs">
          <TestTube size={20} /> Tests
        </button>
        <button onClick={() => navigate('/my-results')} className="flex flex-col items-center text-gray-500 text-xs">
          <Calendar size={20} /> Bookings
        </button>
        <button onClick={() => navigate('/dashboard')} className="flex flex-col items-center text-gray-500 text-xs">
